...
const SubscriptionContext = createContext({
  plan: "free",
  // Free default: no plan and no purchases. Consumers that read access
  // from the default (before a fetch lands) get the fully locked-down shape.
  access: getPlanAccess("free", "active", []),
  loading: true,
  subscription: null,
  purchases: [],
  hasPurchase: () => false,
...
export function SubscriptionProvider({ children }) {
  ...
  const plan = isSubscriptionPaid(subscription)
    ...
    : "free"
  // Pass purchases into getPlanAccess so one-time purchase grants
  // (Documentation Pack, legacy Assessment / Policy Bundle, legacy
  // Individual Policy) OR into the returned flag object. Without this,
  // a free user who bought the $299 Documentation Pack would get
  // canAccessPolicies === false from this hook even though
  // lib/stripe/subscription.js correctly merges the grants. Any
  // component that reads `access` from this context would then show a
  // paywall to a paid user.
